 ...
 * @date 02/01/2023
 * @brief Header that contains macros to run a dynamic vector
 *
 ...
 *
 * This file is licensed by MIT,
 * with Copyright (c) 2015 Evan Teran.
 *
 * It has been edited and redistributed under
 * GPL license with Copyright (c) 2023 Marco Mihai Condrache.
 */

#ifndef SNAKE_CVECTOR_H
#define SNAKE_CVECTOR_H
 ...
 * allow This defines to be overridden.
 */
...
/**
 * @brief cvector_splice - copies a part of the vector
 * to another.
 *
 * @param vec - the vector
 * @param start - where to begin the copy
 * @param count - how many elements to copy
 */
#define cvector_splice(vec, start, count)                                                                                  \
    if((vec)){                                                                                                             \
        memmove((vec) + (start), (vec) + ((start) + (count)), (cvector_size((vec)) - (start) - (count)) * sizeof(*(vec))); \
...
/**
 * @brief cvector_end - returns an iterator to the last element of the vector
 * @param vec - the vector
 * @return a pointer to the last element (or NULL)
 */
#define cvector_last(vec) \
    ((vec) ? (vec) + (cvector_size(vec) - 1) : NULL)
...
/**
 * @brief cvector_pop_back - removes and returns the last element from the vector
 * @param vec - the vector
 * @return void
 */
#define cvector_pop_return(vec)                                                         \
    (vec)[cvector_size(vec) - 1]; cvector_set_size((vec), cvector_size(vec) - 1);
...
#define cvector_copy(from, to)                                          \
    do {                                                                \
        if ((from)) {                                                   \
            ...
            memmove((to), (from), cvector_size(from) * sizeof(*(from))); \
        }                                                               \
    } while (0)

/**
 * @brief cvector_push_vector - pushes a vector from another
 * @param vec - the original vector
 * @param from - source of the push
 * @return void
 */
#define cvector_push_vector(vec, from)\
    do {                              \
        if (!(vec) && (from)){                  \
            cvector_copy(from, vec);                              \
        }else if((from)) {                                                  \
            cvector_grow((vec), cvector_size((from)) + cvector_size((vec)));     \
            memset(cvector_end((vec)) + !cvector_end((vec)), 0, sizeof(*(vec)));                   \
            memmove(cvector_end((vec)) + !cvector_end((vec)), (from), cvector_size(from) * sizeof(*(from))); \
            cvector_set_size((vec), cvector_size((from)) + cvector_size((vec)));\
        }                                                               \
    } while (0)
...
/**
 * @brief cvector_reverse - Reverses the elements of the vector.
 * @param vec - the vector
 * @return void
 */
#define cvector_reverse(vec) \
    do{                      \
        if ((vec)){          \
            char * t = (char *) cvector_clib_malloc(sizeof(*(vec)));                 \
            int i__ = cvector_size((vec)) / 2; \
            while(i__--){    \
                memmove(t, (vec) + i__, sizeof(*(vec)));                            \
                memmove((vec) + i__, (vec) + cvector_size((vec)) - (i__ + 1), sizeof(*(vec)));\
                memmove((vec) + cvector_size((vec)) - (i__ + 1), t, sizeof(*(vec)));\
            }                \
            cvector_clib_free(t);\
        }\
    } while (0)
...
/**
 * @brief cvector_reverse_for_each_in - for header to iterate backwards over vector each element's address
 * @param it - iterator of type pointer to vector element
 * @param vec - the vector
 * @return void
 */
#define cvector_reverse_for_each_in(it, vec) \
    if((vec))                                         \
        for ((it) = cvector_end(vec) - 1; (it) >= cvector_begin(vec); (it)--)

/**
 * @brief cvector_erase_element - function to erase an element without knowing the index
 * @param vec - the vector
 * @param element - the searched element
 * @param cmp - function used to compare elements
 * @return void
 */
#define cvector_erase_element(vec, element, cmp) \
    do {                                    \
        int index__ = -1;                  \
        for(size_t i__ = 0; i__ < cvector_size((vec)); i__++) {\
            if((cmp)((element), (vec)[i__])) {     \
                index__ = (int) i__;    \
                break;\
            }           \
        }                                   \
        if(index__ != -1)                     \
            cvector_erase((vec), index__);\
    } while (0);
